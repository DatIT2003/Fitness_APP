...
export default function Header({ displayName, onPressAvatar }) {
  ...
  const fetchAvatar = async () => {
    try {
      ...
      if (storedUri) {
        const uri = JSON.parse(storedUri); // Phân tích URI đã lưu
        updateAvatar(uri);
      }
    } catch (error) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: '#F1BE48' }]}>
      <TouchableOpacity
        ...
        {/* Sử dụng MaterialIcons và đặt màu thành theme.textColor */}
        <MaterialIcons name="menu" size={24} />
      </TouchableOpacity>
      <Text style={[styles.usernameText, { color: theme.textColor }]}>
          Chào mừng bạn đến với Fitness <Text style={styles.highlightedText}>{displayName}</Text>
      </Text>
...
const styles = StyleSheet.create({
  container: {
    ...
    marginBottom: 0,
    marginHorizontal: 0,
    backgroundColor: '#F1BE48', // Đặt màu nền thành #F1BE48
  },
  icon: {
    marginTop: 30,
    marginLeft: 10,
